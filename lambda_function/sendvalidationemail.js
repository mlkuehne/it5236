...
exports.handler = (event, context, callback) => {
	...
	conn.connect(function(err) {
		...
		conn.query(sql, [event.validationid, event.userid, event.email], function (err, result) {
		  	if (err) {
				// This should be a "Internal Server Error" error
				callback(formatErrorResponse('INTERNAL_SERVER_ERROR', [err]));
		  	} else {
		    	console.log("Message sent to new user email");
		    	callback(null,"validation email successfully sent");
		    } //good code count
