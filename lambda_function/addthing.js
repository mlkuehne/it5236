...
exports.handler = (event, context, callback) => {
	...
	conn.connect(function(err) {
		...
		conn.query(sql, [event.thingid, event.name, event.userid, event.attachmentid, event.registrationcode], function (err, result) {
		  	if (err) {
				// This should be a "Internal Server Error" error
				callback(formatErrorResponse('INTERNAL_SERVER_ERROR', [err]));
		  	} else {
		    	
		    	console.log("Thing insert successful");
		    	callback(null, "Thing insert successful");
